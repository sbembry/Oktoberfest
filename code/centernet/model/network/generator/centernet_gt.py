...
class CenterNetGT(object):
    ...
    def generate(config, y):
        ...
        for out in y:
            ...
            boxes, classes = out[:,:4], out[:,4]
            actual_boxes = classes > -1
            boxes, classes = boxes[actual_boxes].float(), classes[actual_boxes].int()
            num_boxes = boxes.shape[0]#.tensor.shape[0]
            boxes *= box_scale
